...
async function getEpisode(orgSlug: string, podcastSlug: string, episodeId: string) {
  const res = await fetch(`${API}/public/podcasts/${orgSlug}/${podcastSlug}/episodes/${episodeId}`, { next: { revalidate: 300 } });
  if (!res.ok) return null;
  return res.json().then((d) => d.data);
}

export async function generateMetadata({ params }: { params: { org_slug: string; podcast_slug: string; episode_id: string } }): Promise<Metadata> {
  const [podcast, episode] = await Promise.all([
    getPodcast(params.org_slug, params.podcast_slug),
    getEpisode(params.org_slug, params.podcast_slug, params.episode_id),
  ]);
...
export default async function EpisodePage({ params }: { params: { org_slug: string; podcast_slug: string; episode_id: string } }) {
  const [podcast, episode] = await Promise.all([
    getPodcast(params.org_slug, params.podcast_slug),
    getEpisode(params.org_slug, params.podcast_slug, params.episode_id),
  ]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href={`/p/${params.org_slug}/${params.podcast_slug}`} className="mt-4 inline-flex items-center gap-1.5 text-sm text-accent hover:underline">
  ...
  const jsonLd = {
    ...
    partOfSeries: {
      "@type": "PodcastSeries",
      name: podcast.title,
      url: `https://app.freedompodcasting.com/p/${params.org_slug}/${params.podcast_slug}`,
    },
  ...
  return (
    <div className="min-h-screen bg-ink-950">
      ...
      <div className="max-w-2xl mx-auto px-6 py-16">
        {/* Back link */}
        <Link href={`/p/${params.org_slug}/${params.podcast_slug}`} className="inline-flex items-center gap-1.5 text-sm text-ink-500 hover:text-accent transition-colors mb-8">
